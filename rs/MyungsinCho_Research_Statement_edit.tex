...
Blumberg and Mandell proved a stronger spectrum-level duality at every odd prime using Anderson duality, a spectral refinement of Pontrjagin duality for abelian groups \cite{MR4121155}. % Anderson duality라는 도구를 이용한게 아니고, Anderson duality라는 언어로 fomulate했다는 뉘앙스가 들어갔으면 좋겠다.
...
Let \(F\) be a number field. The completion sequence is self Anderson dual.
The bottom row is obtained by applying \(I_{\mathbb Z_2}\) to the top row.
...
% Top row
\node[main] (global) at (-5.2,1.35)   
	{$L_{K(1)}K\bigl(\mathcal O_F[\tfrac12]\bigr)$};

\node[local] (local) at (0,1.35) 
	{$\prod\limits_{\nu\mid2}L_{K(1)}K(F_\nu)$};

\node[main] (fiber) at (5.2,1.35) 
	{$\Sigma\Fib(\kappa)$};

\draw[seq]   (global) --  node[above,font=\scriptsize] {$\kappa$}  (local);

\draw[seq] (local) -- (fiber);

\node[  font=\footnotesize,  text=black!60] at (0,2.25)  {Completion sequence};

% Bottom row
\node[main] (Ifiber) at (-5.2,-1)   
	{$I_{\mathbb Z_2} \bigl(\Sigma\Fib(\kappa)\bigr)$};

\node[local] (Ilocal) at (0,-1)
	 {$I_{\mathbb Z_2}\! \Big( \prod\limits_{\nu\mid2}L_{K(1)}K(F_\nu)  \Big)$};

\node[main] (Iglobal) at (5.2,-1)
	  {$I_{\mathbb Z_2}L_{K(1)} K \bigl(\mathcal O_F[\tfrac12]\bigr)$};

\draw[seq] (Ifiber) -- (Ilocal);
\draw[seq] (Ilocal) --  node[above,font=\scriptsize] {$I_{\bZ_2}\kappa$} (Iglobal);

\node[  font=\footnotesize,  text=black!60] at (0,-2)  {Anderson dual sequence};

% Main duality equivalences
\draw[main equiv]
  (global.south) --
  node[right,font=\small,text=blue!55!black] {$\simeq$}
  (Ifiber.north);
...
\draw[local equiv]
  (local.south) --
  node[  left=2pt,    align=right,    font=\scriptsize,    text=black!45  ] {$\simeq$}
  node[  right=2pt,    align=right,    font=\scriptsize,    text=black!45  ] {self-duality}
  (Ilocal.north);

% Highlight the two equivalences proved by the theorem
...
% Dualize the entire sequence
\draw[   -{Latex[length=2.3mm]},   thick,   black!45 ]
  (7.15,1)
  to[out=-15,in=15]   node[     right=3pt,     align=center,     font=\scriptsize,     text=black!50   ] {apply\\\(I_{\mathbb Z_2}\)}
  (7.15,-0.5);
...
The following figure summarizes the passage from arithmetic duality to its homotopical refinement.
\begin{figure}[h]
\centering
\begin{tikzcd}[column sep=4em, row sep=1em]
  ...
  {H^s_{\textup{\'et}}\!\big(\calO_F[\tfrac{1}{p}];\,\bZ/p^k(\tfrac{t}{2})\big)}
    ...
    &&
  {H^{3-s}_c\!\big(\calO_F[\tfrac{1}{p}];\,\bZ/p^k(1-\tfrac{t}{2})\big)}
...
linked by the descent spectral sequence.
}
\label{fig:duality}
\end{figure}
...
\begin{theorem}[\cite{Cho}]
Let \(k\) be a 2-adic local field. The hyperbolic sequence is self Anderson dual.
The bottom row is obtained by applying \(I_{\mathbb Z_2}\) to the top row.
\begin{center}
\begin{tikzpicture}[
  >=Latex,
...
% Top row
\node[main] (global) at (-5.2,1.35)   
	{$L_{K(1)}\Fib(\operatorname{hyp}_k)$};
...
\draw[seq]   (global) -- (local);

\draw[seq] (local) -- node[above,font=\scriptsize] {$\operatorname{hyp}$}   (fiber);

\node[  font=\footnotesize,  text=black!60] at (0,2.25)  {Hyperbolic sequence};

% Bottom row
\node[main] (Ifiber) at (-5.2,-1)   
	{$I_{\mathbb Z_2} L_{K(1)}GW(k)$};

\node[local] (Ilocal) at (0,-1)
	 {$I_{\mathbb Z_2}\! \Big( L_{K(1)}K(k)  \Big)$};

\node[main] (Iglobal) at (5.2,-1)
	  {$I_{\mathbb Z_2}L_{K(1)} \Fib(\operatorname{hyp}_k)$};

\draw[seq] (Ifiber) -- node[above,font=\scriptsize] {$I_{\bZ_2}\operatorname{hyp}_k$} (Ilocal);
\draw[seq] (Ilocal) --   (Iglobal);
...
Let \(F\) be a number field. The following diagram of completion
cofiber sequences is self-dual under \(2\)-adic Anderson duality.
The duality acts by a half-turn, interchanging the \(U\)- and
Grothendieck--Witt-theoretic rows. Its middle slice is the
\(K\)-theoretic Tate--Poitou duality theorem.
...
\begin{tikzpicture}[
  ...
  seq/.style={->,semithick,black!75},
  vertical/.style={->,semithick,black!55},
  rotation/.style={
    -{Latex[length=2.4mm]},
    very thick,
    blue!55!black
...
% U-theory row
\node[term] (Ug) at (-4.75,2.05)
  {$L_{K(1)}U\bigl(\mathcal O_F[\tfrac12]\bigr)$};

\node[term] (Ul) at (0,2.05)
  {$\prod\limits_{\nu\mid2}L_{K(1)}U(F_\nu^\wedge)$};

\node[term] (Uc) at (4.75,2.05)
  {$\Sigma\Fib(\kappa'')$};

\draw[seq]
  (Ug) -- node[above,font=\scriptsize] {$\kappa''$} (Ul);
\draw[seq] (Ul) -- (Uc);

% K-theory row
\node[term] (Kg) at (-4.75,0)
  {$L_{K(1)}K\bigl(\mathcal O_F[\tfrac12]\bigr)$};

\node[term] (Kl) at (0,0)
  {$\prod\limits_{\nu\mid2}L_{K(1)}K(F_\nu^\wedge)$};

\node[term] (Kc) at (4.75,0)
  {$\Sigma\Fib(\kappa)$};

\draw[seq]
  (Kg) -- node[above,font=\scriptsize] {$\kappa$} (Kl);
\draw[seq] (Kl) -- (Kc);

% Grothendieck--Witt-theory row
\node[term] (GWg) at (-4.75,-2.05)
  {$L_{K(1)}GW\bigl(\mathcal O_F[\tfrac12]\bigr)$};

\node[term] (GWl) at (0,-2.05)
  {$\prod\limits_{\nu\mid2}L_{K(1)}GW(F_\nu^\wedge)$};

\node[term] (GWc) at (4.75,-2.05)
  {$\Sigma\Fib(\kappa')$};

\draw[seq]
  (GWg) -- node[above,font=\scriptsize] {$\kappa'$} (GWl);
\draw[seq] (GWl) -- (GWc);
...
\draw[vertical]
  (Kg) -- node[right,font=\scriptsize] {$\operatorname{hyp}$} (GWg);
\draw[vertical]
  (Kl) -- node[right,font=\scriptsize] {$\operatorname{hyp}$} (GWl);
\draw[vertical] (Kc) -- (GWc);

% The full diagram and its K-theoretic slice
\begin{scope}[on background layer]
\node[
  fit=(Ug)(Uc)(GWg)(GWc),
  rounded corners=9pt,
...
\node[
  ...
  {A larger self-dual completion diagram};
...
\node[
  ...
  text=black!55
] at ($(slice.north)+(-2.3,0.12)$)
  {K-theoretic Tate--Poitou duality};

% The half-turn induced by Anderson duality
\draw[rotation]
  ($(whole.north east)+(-0.10,-0.55)$)
  to[out=5,in=-5,looseness=1.35]
  node[
    right=5pt,
    align=center,
    font=\scriptsize,
    text=blue!50!black
  ] {\(2\)-adic\\Anderson duality}
  ($(whole.south east)+(-0.10,0.55)$);

\draw[rotation]
  ($(whole.south west)+(0.10,0.55)$)
  to[out=185,in=175,looseness=1.35]
  ($(whole.north west)+(0.10,-0.55)$);
...
While duality describes structural relationships in Real algebraic K-theory, explicit calculations usually require a different set of tools. 
In joint work with Teena Gerhardt, Liam Keenan, Juan C. Moreno, and J.D. Quigley, I develop \emph{homological Real trace methods} for computing Real trace invariants \cite{CGKMQ}. 
Real trace methods approach $KR$ through more computable invariants built from Real topological Hochschild homology $\operatorname{THR}$.
The natural \(O(2)\)-action on $\operatorname{THR}$ gives rise to more refined trace invariants, including $\operatorname{TCR}$ and $\operatorname{TCR}^-$.

Our main tool is a spectral sequence that uses the homology of \(\operatorname{THR}\) to compute the continuous homology of \(\operatorname{TCR}^-\), while retaining multiplicative structure and equivariant power operations.
As first applications, we carry out these computations for Real bordism \(MU_{\mathbb R}\), Real Brown--Peterson theory \(BP_{\mathbb R}\), and several truncated variants of \(BP_{\mathbb R}\), which form a fundamental family of examples in equivariant chromatic homotopy theory.
...
\[
H_\star\bigl(\operatorname{THR}(A); \ul{\bF}_2\bigr)\langle \bar y\rangle \Longrightarrow H^c_\star\bigl(\operatorname{TCR}^-(A); \ul{\bF}_2\bigr),
\]
...
Our construction is a Real refinement of the homological trace methods developed by Bruner and Rognes \cite{MR2153113}. In the nonequivariant setting, their methods became an important tool in the study of Segal conjectures for topological Hochschild homology of $MU$ and $BP$. 
Guided by this precedent, my long-term goal is to use homological Real trace methods to formulate and study a Real analogue of the Segal conjecture, beginning with the trace invariants of \(MU_{\mathbb R}\) and \(BP_{\mathbb R}\). 
This would extend the present calculations toward a broader understanding of \(\operatorname{TCR}\) and Real algebraic K-theory.

Our computations use the multiplicative and power-operation structure carried by the homology of \(THR\).
This makes the algebra of genuine equivariant operations an essential input to Real trace methods.
It leads to the broader question of which collections of equivariant operations can occur together.
