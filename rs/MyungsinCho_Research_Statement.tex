...
A central idea in modern homotopy theory is the notion of localization, such as $\bA_1$-locality in the motivic setting. 
When translated to complex analytic geometry, a space is $\bC$-local if every holomorphic map from $\bC$ into it is constant. 
This exactly recovers the classical geometric condition of Brody hyperbolicity. 
For example, a compact Riemann surface of genus $g\ge 2$ is hyperbolic, which serves as a geometric analogue of homotopical rigidity. In this sense, hyperbolicity can be natively understood as a localization condition on categories of simplicial presheaves and stacks.
 
Demailly’s approach to hyperbolicity provides a systematic framework by translating curvature conditions into cohomological language. 
Building on this perspective and the work of Borghesi and Tomassini \cite{MR3673667}, who formalized hyperbolicity for simplicial presheaves, my recent joint work with Gunhee Cho \cite{CC} investigates the obstructions to $\bC$-locality using the theory of jet differentials. 
We prove that the \emph{Green--Griffiths--Demailly thresholds}, which govern invariant jet differentials, remain unchanged when passing from a compact complex manifold to a \emph{smooth analytic Deligne--Mumford stack}.
...
\end{theorem}
Viewing hyperbolicity strictly through the lens of $\bC$-locality provides a concrete model for applying homotopy-theoretic invariance to analytic geometry. 
This stack-theoretic approach forms the foundation for our ongoing work extending these localization principles to study hyperbolicity and deformation theory in the broader context of higher and derived stacks.
